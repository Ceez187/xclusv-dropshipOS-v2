...
const RESPONSE_SHAPE = `{
  ...
  "searchKeywordZh": string,      // Specific Chinese search keyword for 1688/Taobao. Include concrete distinguishing details (material, size/capacity, color, key function/feature) instead of a generic category name — phrase it the way a real seller would title this exact product on 1688/Taobao, so the search surfaces closely matching listings instead of broad, unrelated results. Never output a single generic word (e.g. just "水杯") when the product has any distinguishing details available.
  "searchKeywordEn": string       // English keyword with the same level of distinguishing detail as searchKeywordZh, for AliExpress and for reference
}`
