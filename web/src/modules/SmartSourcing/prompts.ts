...
const RESPONSE_SHAPE = `{
  ...
  "suppliers": [ { "type": string, "moq": string, "notes": string, "authenticity": "verified" | "risk" | "unknown" } ], // list 5-6 distinct supplier options, spanning 1688, Taobao, AliExpress, and a Basetao agent — include more than one option per channel where realistic (e.g. two different 1688 supplier profiles at different price/quality tiers). Prioritize suppliers with low or no MOQ (1 unit / no minimum) since this is for dropshipping with no upfront inventory — sort the list from lowest MOQ to highest. Set "authenticity" per supplier: "verified" for a known/likely genuine manufacturer or a verified (e.g. Gold Supplier) factory-direct source selling the authentic/original product; "risk" for an unverified trading company/reseller where counterfeit or replica risk is plausible (common for branded or trademarked items); "unknown" only when there's truly not enough signal either way. Explain the reasoning briefly in "notes", and also note there when a supplier is generally known for low return/complaint rates and high repeat-order rates — you have no live return-rate data, so base both judgments on general supplier-type reputation (e.g. factory-direct vs. reseller) rather than fabricated statistics
