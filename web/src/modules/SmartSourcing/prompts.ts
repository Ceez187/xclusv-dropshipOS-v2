...
const RESPONSE_SHAPE = `{
  ...
  "qualityIndicators": string[],  // 4-6 short, concrete signals of a trustworthy factory-direct supplier FOR THIS product category specifically (not generic advice) — e.g. transaction volume threshold, "工厂" factory-direct vs. trading company, response rate, years active on 1688, sample policy, the specific spec detail a professional seller in this category would list (fabric GSM, material grade, etc). One line each, no fluff.
  "redFlags": string[],           // 4-6 short, concrete red flags to avoid for THIS product category specifically — missing spec details, stock-only photos, suspiciously low MOQ with no sample fee, review patterns, etc. One line each.
  "keywordVariants": [ { "chinese": string, "pinyin": string, "english": string } ] // 3-5 rows of alternate ways a shopper might phrase this search (different specificity/synonyms), each with pinyin so someone who can't read Chinese can still recognize/say it. Same 2-5 word discipline as searchKeywordZh — these are alternates to try, not longer descriptions.
}`
